...
public class PlayerDBController implements NetworkThread.MessageListener {
    ...
    private void refreshGrid() {
        ...
        List<Player> filtered = squad.stream()
                .filter(p -> currentPosition.equals("ALL") ||
                        p.getPosition().equals(currentPosition))
        ...
        countLabel.setText(filtered.size() + " player" +
                (filtered.size() != 1 ? "s" : ""));
    }
        ...
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource(
                            "/org/buet/fantasymanagerxi/fxml/player-detail.fxml")
            );
            Parent root = loader.load();
            PlayerDetailController ctrl = loader.getController();
            ctrl.setPlayer(p);
    ...
    private void openMarket() {
        // Request the latest market listings from the server
    ...
    public void onMessageReceived(MarketMessage msg) {
        switch (msg.getType()) {
            ...
            case MARKET_UPDATE -> {
                // Navigate to the transfer market screen
                try {
                    FXMLLoader loader = new FXMLLoader(
                            getClass().getResource(
                                    "/org/buet/fantasymanagerxi/fxml/transfer-market.fxml")
                    );
                    Parent root = loader.load();
                    Stage stage = (Stage) playerGrid.getScene().getWindow();
                    stage.setScene(new Scene(root, 1100, 720));
    ...
    public void onConnectionFailed(String reason) {
        countLabel.setText("Connection lost: " + reason);
    }
